...
final class CalculatorViewController: UIViewController, ReactorKit.View {
    ...
    // MARK: - Initialization
    init(reactor: CalculatorReducer) {
    ...
    func bind(reactor: CalculatorReducer) {
        // State 바인딩
        reactor.state
            .map { $0.display }
    ...
    private func setupButtons(reactor: CalculatorReducer) {
        // 숫자 버튼들
        for i in 0...9 {
            if let button = view.viewWithAccessibilityIdentifier("number_button_\(i)") as? UIButton {
                button.rx.tap
                    .map { CalculatorReducer.Action.inputNumber(i) }
        ...
        if let addButton = view.viewWithAccessibilityIdentifier("operation_button_add") as? UIButton {
            addButton.rx.tap
                .map { CalculatorReducer.Action.setOperation(.add) }
                .bind(to: reactor.action)
                .disposed(by: disposeBag)
        }
        ...
        if let subtractButton = view.viewWithAccessibilityIdentifier("operation_button_subtract") as? UIButton {
            subtractButton.rx.tap
                .map { CalculatorReducer.Action.setOperation(.subtract) }
                .bind(to: reactor.action)
                .disposed(by: disposeBag)
        }
        ...
        if let multiplyButton = view.viewWithAccessibilityIdentifier("operation_button_multiply") as? UIButton {
            multiplyButton.rx.tap
                .map { CalculatorReducer.Action.setOperation(.multiply) }
                .bind(to: reactor.action)
                .disposed(by: disposeBag)
        }
        ...
        if let divideButton = view.viewWithAccessibilityIdentifier("operation_button_divide") as? UIButton {
            divideButton.rx.tap
                .map { CalculatorReducer.Action.setOperation(.divide) }
                .bind(to: reactor.action)
                .disposed(by: disposeBag)
        }
        ...
        if let equalsButton = view.viewWithAccessibilityIdentifier("equals_button") as? UIButton {
            equalsButton.rx.tap
                .map { CalculatorReducer.Action.calculate }
                .bind(to: reactor.action)
                .disposed(by: disposeBag)
        }
        ...
        if let clearButton = view.viewWithAccessibilityIdentifier("clear_button") as? UIButton {
            clearButton.rx.tap
                .map { CalculatorReducer.Action.clear }
                .bind(to: reactor.action)
                .disposed(by: disposeBag)
        }
    }
    ...
    // MARK: - Alert Handling
    private func handleAlert(_ alertType: CalculatorReducer.AlertType?) {
...
@available(iOS 17.0, *)
#Preview {
    CalculatorViewController(reactor: CalculatorReducer())
}
